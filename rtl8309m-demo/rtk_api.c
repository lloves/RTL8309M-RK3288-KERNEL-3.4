 ...
 *      void
 * Note:
 *      The API can release the resource.
 */
void rtk_switch_release(void)
{
	smiRelease();
}
...
void get_phy_status_get(void) {
    int i = 0;
    for(i=0; i < 9; i++) {
        rtk_port_linkStatus_t linkStatus;
        rtk_port_speed_t speed;
        rtk_port_duplex_t duplex;
        int ret = rtk_port_phyStatus_get(i, &linkStatus, &speed, &duplex);
        if(ret != 0) {
            printf("get port %d status failed.\n", i);
        }
        // printf(": phy = %d, link = %s,speed=%s duplex =%s\n", i, linkStatus==PORT_LINKUP?"up":"down", speed==PORT_SPEED_100M?"100M":"10M", duplex==PORT_FULL_DUPLEX?"FULL$
        // printk("phy %d status linkStatus %d, speed %d, duplex %d.\n", linkStatus, speed, duplex);
    }
    return;
}
