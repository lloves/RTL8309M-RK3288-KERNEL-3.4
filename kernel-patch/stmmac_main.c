...
#include "stmmac.h"
#include "../eth_mac.h"
// add for switch(RTL8309M) support
// #include "RTL8309M/rtk_api_ext.h"
...
// add for switch(RTL8309M) support
/*
int init_rtl8309m_switch(void)
{
    printk("init RTL8309M.\n");
    rtk_switch_init();
    rtk_vlan_init();
    // config rmii. port 8
    enablePortlink();
...
static int stmmac_init_phy(struct net_device *dev)
{
   ...
   if (interface == PHY_INTERFACE_MODE_RMII) {
       ...
       if (!phydev || IS_ERR(phydev)) {
           ...
           printk("fixed_phy_register rtl8309 error\n");
           return -ENODEV;
       }
       ...
       ret = phy_connect_direct(dev, phydev, stmmac_adjust_link,
                    interface);
       if (ret) {
            printk("could not attach to PHY rtl8309\n");
       ...
       phydev->link = 0;
       priv->phydev = phydev;
       printk("rtl8309 phy addr success. down\n");
       return 0;
   }
#endif
...
static int stmmac_open(struct net_device *dev)
{
	...
	if (priv->pcs != STMMAC_PCS_SGMII && priv->pcs != STMMAC_PCS_TBI &&
		...
		if (ret) {
			printk("%s: Cannot attach to PHY (error: %d)\n",
			       __func__, ret);
			goto open_error;
		}
	...
	if (ret < 0) {
		printk("%s: DMA initialization failed\n", __func__);
		goto open_error;
	}
	...
	if (unlikely(ret < 0)) {
		printk("%s: ERROR: allocating the IRQ %d (error: %d)\n",
	...
	if (priv->wol_irq != dev->irq) {
		ret = request_irq(priv->wol_irq, stmmac_interrupt,
				  IRQF_SHARED, dev->name, dev);
		if (unlikely(ret < 0)) {
			printk("%s: ERROR: allocating the WoL IRQ %d (%d)\n",
	...
	if (priv->lpi_irq != -ENXIO) {
		ret = request_irq(priv->lpi_irq, stmmac_interrupt, IRQF_SHARED,
				  dev->name, dev);
		if (unlikely(ret < 0)) {
			printk("%s: ERROR: allocating the LPI IRQ %d (%d)\n",
...
*/
    ...
    // add for support RTL8309M switch.

    // init_rtl8309m_switch();
